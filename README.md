...
| ボタン | 操作 | 動作 |
|--------|------|------|
| USER | 短押し | 自分のピクチャ確認（10 秒で流体へ復帰）。閲覧モード中は次の履歴へ |
| USER | 長押し(0.8s) | 受信履歴の閲覧モード 開始/終了 |
| BOOT | 短押し | OTA モードへ移行（LED 緑） |
| BOOT | 長押し(5s) | 全データ・設定を初期化（factory reset） |
